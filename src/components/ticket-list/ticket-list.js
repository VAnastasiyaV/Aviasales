...
class TicketList extends React.PureComponent {
   ...
   getFiltratedTickets = () => {
      ...
      filters.forEach(filter => {
         selectedTickets = selectedTickets.concat(allTickets.filter(ticket => ticket.stopNames1.length === filter && ticket.stopNames2.length === filter
         ));
      })
