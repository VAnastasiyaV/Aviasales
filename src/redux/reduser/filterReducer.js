...
const getTransfers = (numberTransfers, transfersFilterArr) => {
   ...
   arrNumberTransfers.map(numberTransfer => {
      ...
      if (numberTransfer === -1) {
         transfersFilterArr.includes(-1) ? transfersFilterArr = [] : transfersFilterArr = [-1, 0, 1, 2, 3];
      } else {
         ...
         if (transfersFilterArr.includes(numberTransfer)) {
            ...
            transfersFilterArr = transfersFilterArr.includes(-1)
               ...
               : transfersFilterArr;
            // для остальных фильтров - устанавливаем соответствующую галочку и галочку "все" - если выбраны четыре оставшихся фильтра
         } else {
